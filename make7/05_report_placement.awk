...
function generate_placement_report(    inst, model, p_vcc, p_gnd, coord_p1, coord_vcc, coord_gnd, p1_str, vcc_str, gnd_str, target_ci, cap_p1, cap_p2) {
    ...
    # --------------------------------------------------------------------------
    # SEÇÃO 1: MAPA GEOMÉTRICO DE ALOCAÇÃO DE CIRCUITOS INTEGRADOS REAIS (FUROS INTEIROS)
    # --------------------------------------------------------------------------
    ...
    for (inst in COMP_INST_MODEL) {
        ...
        delete coord_p1
        get_pin_grid_coords(inst, 1, coord_p1)
        p1_str = "1@(" coord_p1["X"] "," coord_p1["Y"] ")"
        
        p_gnd = DB_NAME_TO_PIN[model, "GND"]
        if (p_gnd != "") {
            delete coord_gnd
            get_pin_grid_coords(inst, p_gnd, coord_gnd)
            gnd_str = p_gnd "@(" coord_gnd["X"] "," coord_gnd["Y"] ")"
        } else {
            gnd_str = "N/A"
        }
        ...
        if (p_vcc != "") {
            delete coord_vcc
            get_pin_grid_coords(inst, p_vcc, coord_vcc)
            vcc_str = p_vcc "@(" coord_vcc["X"] "," coord_vcc["Y"] ")"
        } else {
            vcc_str = "N/A"
        }
    ...
    # --------------------------------------------------------------------------
    # SEÇÃO 2: SUMÁRIO DE VALIDAÇÃO DRC ELÉTRICO
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # SEÇÃO 4: LISTAGEM EXCLUSIVA DE CAPACITORES COM MEDIÇÕES DE PONTAS DE 0.2"
    # --------------------------------------------------------------------------
    ...
    for (inst in COMP_INST_MODEL) {
        if (inst ~ /^C_/) {
            target_ci = substr(inst, 3)
            
            # Interroga a função geométrica de pinos para extrair as posições inteiras das duas pontas do componente DSC
            delete cap_p1; delete cap_p2
            get_pin_grid_coords(inst, 1, cap_p1)
            get_pin_grid_coords(inst, 2, cap_p2)
