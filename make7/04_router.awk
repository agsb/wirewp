...
function run_manhattan_routing(    u_net, total_pts, i, j, visited, current, next_pt, min_d, d, cx, cy, nx, ny, p1, p2, part1, pin1, part2, pin2, split_buffer_1, split_buffer_2) {
    ...
    for (u_net in NET_MEMBERS_COUNT) {
        ...
        for (i = 1; i < total_pts; i++) {
            ...
            delete p1
            get_pin_grid_coords(part1, pin1, p1)
            cx = p1["X"]; cy = p1["Y"]
            ...
            for (j = 1; j <= total_pts; j++) {
                if (!visited[j]) {
                    ...
                    delete p2
                    get_pin_grid_coords(part2, pin2, p2)
                    nx = p2["X"]; ny = p2["Y"]
            ...
            if (next_pt != -1) {
                ...
                if (PIN_Z_LEVEL[split_buffer_1[1], split_buffer_1[2]] > 2) {
                    print "[ERRO DRC CRÍTICO]: Componente " split_buffer_1[1] " pino " split_buffer_1[2] " violou o limite mecânico! [OVERWRAP: MAIS DE 2 FIOS NO PINO!]" > "/dev/stderr"
                }
                if (PIN_Z_LEVEL[split_buffer_2[1], split_buffer_2[2]] > 2) {
                    print "[ERRO DRC CRÍTICO]: Componente " split_buffer_2[1] " pino " split_buffer_2[2] " violou o limite mecânico! [OVERWRAP: MAIS DE 2 FIOS NO PINO!]" > "/dev/stderr"
                }
