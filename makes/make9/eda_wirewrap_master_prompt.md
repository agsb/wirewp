...
O ecossistema é estruturado em 9 arquivos lógicos sequenciais. Todos os cálculos matemáticos de loops, molas, snap e roteamento rodam estritamente dentro do bloco único END localizado apenas no último arquivo (09_execute.awk). Os módulos de 01 a 08 contêm apenas funções e regras de registros, sendo proibidos de usar o comando next dentro de funções chamadas no END para evitar erros de contexto escalar. Os arquivos de entrada são delimitados por espaços simples e limpos de caracteres \r.
...
- Aplica o filtro de fiação de bypass dos capacitores automáticos para não poluir o guia prático do montador.
