...
function run_manhattan_routing(    u_net, total_pts, i, j, visited, current, next_pt, min_d, d, cx, cy, nx, ny, p1, p2, part1, pin1, part2, pin2, split_buffer_1, split_buffer_2) {
    ...
    for (u_net in NET_MEMBERS_COUNT) {
        ...
        for (i = 1; i < total_pts; i++) {
            ...
            # Divide a string "Componente;Pino" armazenada na memória
            split(NET_MEMBERS[u_net, current], split_buffer_1, ";")
            
            # CORREÇÃO DEFINITIVA: Acessa os índices escalares numéricos [1] e [2] do array fatiado
            ...
            if (next_pt != -1) {
                ...
                # Armazena as strings escalares puras na matriz global estável de rotas
                ...
                # Incrementa e registra o empilhamento físico tridimensional por poste
                ...
                # Alertas estritos contra violação mecânica do teto de Wrap (Máx = 2 fios por poste)
