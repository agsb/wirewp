...
function round_to_ceil_cm(value,    int_part) {
    int_part = int(value); if (value > int_part) return int_part + 1
...
function generate_routing_summary(    i, u_net, prio, p1, p2, dist_grid, dist_inch, dist_cm, dist_slack_cm, part1, pin1, part2, pin2, model1, model2, phys_pin1, phys_pin2, sum_file, txt_color, current_prio_group, z1, z2, z1_label, z2_label, z_alert, final_std_cm, final_slack_cm, wrap_wrap_hardware_allowance_cm, SUM_TOTAL_STD, SUM_TOTAL_SLACK) {
    sum_file = "board_routing_summary.txt"; SUM_TOTAL_STD = 0; SUM_TOTAL_SLACK = 0; wrap_wrap_hardware_allowance_cm = 4.544
    print "==================================================================================================================" > sum_file
    ...
    for (current_prio_group = 1; current_prio_group <= 7; current_prio_group++) {
        for (i = 1; i <= ROUTE_COUNT; i++) {
            ...
            if (part1 ~ /^C_/ || part2 ~ /^C_/) {
                ...
                txt_color = (u_net ~ /^(GND|DGND|VSS|AGND)$/) ? "PRETO" : "VERMELHO"
                z1_label = "BAIXO"; z2_label = "BAIXO"; z_alert = "OK"
            } else {
                model1 = COMP_INST_MODEL[part1]; model2 = COMP_INST_MODEL[part2]
                ...
                z1_label = (z1 == 1) ? "BAIXO" : "CIMA"; z2_label = (z2 == 1) ? "BAIXO" : "CIMA"; z_alert = (z1 > 2 || z2 > 2) ? "OVERWRAP" : "OK"
            }
            ...
            dist_cm = (dist_inch * 2.54) + wrap_wrap_hardware_allowance_cm; dist_slack_cm = dist_cm * 1.20 
            final_std_cm = round_to_ceil_cm(dist_cm); final_slack_cm = round_to_ceil_cm(dist_slack_cm)
            SUM_TOTAL_STD += final_std_cm; SUM_TOTAL_SLACK += final_slack_cm
